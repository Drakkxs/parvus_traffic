...
function C:setAggression(baseAggression, ignorePersonality) -- helper method that sets and sends the ai aggression value
  ...
  local randomAggression = clamp(math.floor((math.random() * math.random()) * 200 + 0.5) / 100, baseAggression, 2)
  randomAggression = clamp(randomAggression + modifier, 0.2, 2)
  self.driver.aggression = randomAggression
  -- speed handled in onCrashDamage()
  getObjectByID(self.veh.id):queueLuaCommand('ai.setAggression('..randomAggression..')') -- slightly randomized aggression (mean 0.3)
end
...
function C:onCrashDamage(data)
  local speedLimit = self.veh.tracking.speedLimit or -1
  if speedLimit >= 0 then
    -- For aggression above one, allow for a higher multiplier
    local speedMultiplier = 1 + math.random() * (self.driver.aggression >=1 and self.driver.aggression or 0.5)
    local maxRandomSpeed = speedLimit * speedMultiplier
    local randomSpeed = math.floor(speedLimit + math.random() * math.random() * (maxRandomSpeed - speedLimit))
    getObjectByID(self.veh.id):queueLuaCommand('ai.setSpeed('..randomSpeed..')')
  end
end
